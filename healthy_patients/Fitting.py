...
Patients = np.load('patients.npy', allow_pickle=True).item()
# Measurement = "Pt3"
...
# %%
# for Measurement in ['Pt3']:
for Measurement in [sys.argv[1]]:
    # take this PWA_ measurement key, which has better quality of the data
    start = time.time()
    
    PWA_data = [w for w in Patients[Measurement].keys() if w.startswith("PWA")] 

    aux   = 0
    i_min = 0 
    for i, m in enumerate(PWA_data):
        OI = Patients[Measurement][m]["data"]["Operator_Index"]
        if OI > aux: 
            aux = OI 
            i_min = i

    PatientPulse = Patients[Measurement][f"PWA{i_min+1}"]["pulse"]["PeriphPulse"]
    PatientCharacteristics = Patients[Measurement][f"PWA{i_min+1}"]["data"]
    ...
    if method == "PSO":
        print("========== PSO fitting ============\n")
        print(f"== Fitting fitting: {paramsToFit} ===")
    ...
    np.save(f"results/{Measurement}_{method}_{add_method}_{i_min+1}.npy", toSave)
    
    print(f"======================================\n"
            f"Patient: {Measurement}, EVALUATION TIME (IN SECONDS): {(time.time() - start):.2f}\n"
            f"======================================\n")
